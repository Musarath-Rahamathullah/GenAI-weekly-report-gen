...
def bedrock_claude_(params,chat_history,system_message, prompt,model_id,image_path=None, handler=None):
    ...
    print(chat_history)
    print("CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC")      
    response = bedrock_runtime.converse_stream(messages=chat_history, modelId=model_id,inferenceConfig={"maxTokens": 4000, "temperature": 0.5,},system=system_message)
    print(response)
...
"input_token":round(st.session_state['input_token']) ,
        ...
        if params['upload_doc']:# or params['s3_objects']:
            ...
            #         file_name=docs
            #         _,extensions=os.path.splitext(file_name)
            #         print(INPUT_BUCKET,INPUT_S3_PATH,docs)
